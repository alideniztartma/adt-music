...
package com.music.adt.adtmusic.updater
...
import com.music.adt.BuildConfig
import com.music.adt.R
import coil3.compose.AsyncImage
import androidx.compose.ui.draw.clip
import androidx.compose.ui.layout.ContentScale
import com.music.adt.adtmusic.updater.downloadmanager.UpdateDownloadWorker
import com.music.adt.adtmusic.updater.downloadmanager.DownloadNotificationManager
...
import com.music.adt.ui.component.ChangelogItem
import com.music.adt.ui.component.leadingItemShape
import com.music.adt.ui.component.middleItemShape
import com.music.adt.ui.component.endItemShape
import com.music.adt.ui.component.detachedItemShape
import com.music.adt.ui.component.AnimatedActionButton
import com.music.adt.ui.component.ExpressiveIconButton
import com.music.adt.ui.component.ErrorSnackbar
...
sealed class ADTUpdateStatus {
    object Idle : ADTUpdateStatus()
    object Checking : ADTUpdateStatus()
    ...
    data class NoUpdate(val version: String) : ADTUpdateStatus()
    data class Error(val message: String) : ADTUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<ADTUpdateStatus>(ADTUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = ADTUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        ADTUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = ADTUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is ADTUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as ADTUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is ADTUpdateStatus.Idle, is ADTUpdateStatus.Checking, is ADTUpdateStatus.NoUpdate, is ADTUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is ADTUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is ADTUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/alideniztartma/adt-music/releases/download/${currentStatus.version}/adt.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is ADTUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is ADTUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is ADTUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
                                ...
                                Column(
                                    ...
                                    currentStatus.changelog.forEach { section ->
                                        ...
                                        section.items.forEachIndexed { index, item ->
                                            val shape = when {
                                                section.items.size == 1 -> com.music.adt.ui.component.detachedItemShape()
                                                index == 0 -> com.music.adt.ui.component.leadingItemShape()
                                                index == section.items.size - 1 -> com.music.adt.ui.component.endItemShape()
                                                else -> com.music.adt.ui.component.middleItemShape()
                                            }
                                            com.music.adt.ui.component.ChangelogItem(
                                                text = item,
                                                shape = shape,
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/alideniztartma/adt-music/releases")
            ...
            if (betaEnabled) {
                try {
                    val nightlyUrl = URL("https://api.github.com/repos/alideniztartma/adt-music/actions/workflows/nightly.yml/runs?status=success&per_page=100")
            ...
            if (isNightlyUpdate && nightlyRunObject != null) {
                ...
                val apkDownloadUrl = "https://nightly.link/alideniztartma/adt-music/workflows/nightly.yml/main/adt-music-gms-nightly.zip"
            ...
            if (targetRelease != null) {
                ...
                if (shouldShow) {
                    ...
                    try {
                        val changelogUrl =
                            URL("https://github.com/alideniztartma/adt-music/releases/download/$tagWithPrefix/changelog.json")
