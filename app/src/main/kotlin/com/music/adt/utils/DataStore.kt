...
operator fun <T> DataStore<Preferences>.get(key: Preferences.Key<T>): T? =
    if (ADTPrefCache.isInitialized()) {
        ADTPrefCache.get(key)
    } else {
...
fun <T> DataStore<Preferences>.get(
    ...
    if (ADTPrefCache.isInitialized()) {
        ADTPrefCache.get(key) ?: defaultValue
    } else {
        runBlocking(Dispatchers.IO) {
            try {
                kotlinx.coroutines.withTimeoutOrNull(1500) {
...
suspend fun <T> DataStore<Preferences>.getAsync(key: Preferences.Key<T>): T? =
    ADTPrefCache.get(key) ?: data.first()[key]

suspend fun <T> DataStore<Preferences>.getAsync(
    key: Preferences.Key<T>,
    defaultValue: T,
): T =
    ADTPrefCache.get(key) ?: data.first()[key] ?: defaultValue
