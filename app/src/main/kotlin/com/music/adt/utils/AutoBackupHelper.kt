...
object AutoBackupHelper {
    ...
    fun performBackup(context: Context, backupType: String): Boolean {
        ...
        try {
            ...
            // Save the temp file to the final destination (public Download/adtmusic)
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                val resolver = context.contentResolver
                val contentValues = ContentValues().apply {
                    put(MediaStore.Downloads.DISPLAY_NAME, fileName)
                    put(MediaStore.Downloads.MIME_TYPE, "application/octet-stream")
                    put(MediaStore.Downloads.RELATIVE_PATH, "Download/adtmusic")
                }
                ...
                val publicDir = File(downloadsDir, "adtmusic")
                if (!publicDir.exists()) {
                    publicDir.mkdirs()
                }
    ...
    fun getAutoBackups(context: Context): List<File> {
        ...
        // 2. Public Downloads/adtmusic folder (using MediaStore on Q+ or File API on legacy)
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                val selectionArgs = arrayOf("Download/adtmusic/%", "Download/adtmusic")
