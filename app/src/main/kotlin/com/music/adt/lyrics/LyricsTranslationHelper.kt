...
import com.music.adt.api.DeepLService
import com.music.adt.api.MistralService
import com.music.adt.api.OpenRouterService
import com.music.adt.api.OpenRouterStreamingService
        ...
        translationJob = scope.launch(Dispatchers.IO) {
            try {
                ...
                if (effectiveApiKey.isBlank()) {
                    _status.value = TranslationStatus.Error(context.getString(com.music.adt.R.string.ai_error_api_key_required))
                    return@launch
                }

                if (lyrics.isEmpty()) {
                    _status.value = TranslationStatus.Error(context.getString(com.music.adt.R.string.ai_error_no_lyrics))
                    return@launch
                }
                ...
                if (nonEmptyEntries.isEmpty()) {
                    _status.value = TranslationStatus.Error(context.getString(com.music.adt.R.string.ai_error_lyrics_empty))
                    return@launch
                }
                ...
                if (targetLanguage.isBlank()) {
                    _status.value = TranslationStatus.Error(context.getString(com.music.adt.R.string.ai_error_language_required))
                    return@launch
                }
                ...
                result.onSuccess { translatedLines ->
                    ...
                    when {
                        ...
                        translatedLines.size < expectedCount -> {
                            ...
                            _status.value = TranslationStatus.Error(context.getString(com.music.adt.R.string.ai_error_unexpected))
                        }
                    }
                    ...
                    val errorMessage = error.message ?: context.getString(com.music.adt.R.string.ai_error_unknown)
                    _status.value = TranslationStatus.Error(errorMessage)
                }
            } catch (e: Exception) {
                if (e !is kotlinx.coroutines.CancellationException && isCompositionActive) {
                    val errorMessage = e.message ?: context.getString(com.music.adt.R.string.ai_error_translation_failed)
                    _status.value = TranslationStatus.Error(errorMessage)
                }
            }
        }
